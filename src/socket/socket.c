...
{
#ifdef _WIN32
    KORALL_LOG(LOG_INFO, "windows platform detected, beginning startup\n");
    ...
    for (int i = 0; i < VERSION_COUNT; i++)
    {
        ...
        KORALL_LOG(LOG_INFO, "trying v%u.%u\n", lobyte, hibyte);

        memset(&wsa_data, 0, sizeof(wsa_data));
        int err = WSAStartup(w_version, &wsa_data);
        if (err != 0)
        {
            KORALL_LOG(LOG_WARN, "WSAStartup failed with error: %d\n", err);
            continue;
        }

        if (LOBYTE(wsa_data.wVersion) != lobyte || HIBYTE(wsa_data.wVersion) != hibyte)
        {
            KORALL_LOG(LOG_ERR, "could not find a usable version of Winsock.dll\n");
            WSACleanup();
            continue;
        }

        KORALL_LOG(LOG_INFO, "Startup success!\n");
        return 0;
    }
