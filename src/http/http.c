...
HTTPError http_request_parse(const char *data, HTTPRequest *req)
{
	...
	if (*data == '\r' && data[1] == '\n' && data[2] == '\0')
		return HTTP_SUCCESS; // no header, no body
...
HTTPError http_request_process_header(const char **str, HTTPRequest *req)
{
	...
	if (header_field == -1 && !http_header_allowed(g_config, field))
		return HTTP_BAD_HEADER; // todo: better error msg
...
HTTPError http_request_process_body(const char *str, HTTPRequest *req)
{
	...
	if (!(method == HTTP_PUT || method == HTTP_PATCH || method == HTTP_POST))
		return HTTP_BODY_NOT_ALLOWED;

	size_t body_len = strlen(str);
	if (body_len > HTTP_REQ_BODY_LEN)
...
HTTPError http_request_process_target_relative(const char *str, HTTPRequest *req)
{
	...
	for (char c = *str; c != '\0'; c = *(++str))
	{
		...
		if (c == '%')
		{
			...
			if (!(is_hex_digit(str[1]) && is_hex_digit(str[2])))
				return HTTP_BAD_REQUEST_TARGET;
			continue;
		}
		if (c == '?')
			break;
		if (!is_valid_path_segment(c))
			return HTTP_BAD_REQUEST_TARGET;
		seg_len++;
	}
	...
	if (*str == '?')
	{
		...
		str++;
		if (*str == '\0')
			return HTTP_BAD_REQUEST_TARGET; // cannot have ".../?" ?
		int part_len = 0;
		bool in_val = false;
		bool in_field = true;
		for (char c = *str; c != '\0'; c = *(++str))
		...
		if (!in_val)
			return HTTP_BAD_REQUEST_TARGET; // must end on value
	}
...
HTTPError http_request_process_target_absolute(const char *str, HTTPRequest *req)
{
	...
	for (char c = *str; c != '\0' && c != '/'; c = *(++str))
	{
		if (c == '.')
		{
			if (label_len > MAX_DOMAIN_LABEL_LEN || label_len < MIN_DOMAIN_LABEL_LEN)
				return HTTP_BAD_REQUEST_TARGET;
			if (prev_c == '-')
				return HTTP_BAD_REQUEST_TARGET;
			label_len = 0;
		...
		if (!is_valid_hostname_label(c))
			return HTTP_BAD_REQUEST_TARGET;
		if (label_len == 0 && c == '-')
			return HTTP_BAD_REQUEST_TARGET; // cant start with -
		prev_c = c;
		label_len++;
		total_label_len++;
	}
	if (total_label_len > MAX_DOMAIN_LEN || prev_c == '.' || prev_c == '-')
		return HTTP_BAD_REQUEST_TARGET;
...
HTTPError http_request_process_target(const char **str, HTTPRequest *req)
{
	HTTPMethod method = req->start_line->method;
	if (method == -1)
		return HTTP_ERROR; // for testing purposes, never actually -1

	char value[MAX_HTTP_URL_LEN + 1] = {0};
	if (fill_string_char(str, value, MAX_HTTP_URL_LEN, ' ') == -1)
		return HTTP_REQUEST_TARGET_TOO_BIG;
...
{
	if (allow_origins == NULL || array_is_empty(allow_origins) || req == NULL || value == NULL)
		return -1;
...
{
	if (allow_methods == NULL || array_is_empty(allow_methods) || value == NULL)
		return -1;
	...
	array_for_loop(i, allow_methods)
	{
		...
		if (meth_len + cur_len > value_len)
			return -1; // exit early if not enough space
...
{
	if (allow_headers == NULL || array_is_empty(allow_headers) || value == NULL)
		return -1;
	// stringify array
	// todo: slow ?
	int i;
	...
	const char *body)
{
	...
	if (body != NULL)
	{
		...
		if (ct_str == NULL)
		{
			KORALL_LOG(LOG_ERR, "response construction failed, content type lookup\n");
			return -1;
		}

		if (korall_response_header_set(res, "Content-Type", ct_str) == -1)
			return -1;
	...
	{
		if (korall_response_header_set(res, "Content-Length", "0") == -1)
			return -1;
	}
	return 0;
}
...
)
{
	...
	if (korall_response_header_set(res, "Upgrade", "websocket") == -1)
		return -1;
	if (korall_response_header_set(res, "Connection", "upgrade") == -1)
		return -1;
	if (korall_response_header_set(res, "Sec-WebSocket-Accept", accept) == -1)
		return -1;
	if (korall_response_header_set(res, "Content-Length", "0") == -1)
		return -1; // ! important
	return 0;
...
int http_response_send(const SOCKET inc_sock, const HTTPResponse *res)
{
	...
	if (data == NULL)
	{
		KORALL_LOG(LOG_ERR, "failed to convert HTTP response to str\n");
...
int korall_request_param_get(const HTTPRequest *req, const char *field, char *value, size_t value_len)
{
	...
	if (fill_string_char(&rt, NULL, 0, '?') == -1)
		return -1;
	// find field
	bool is_case_insensitive = false;
	if (fill_string_str(&rt, NULL, 0, field, is_case_insensitive) == -1)
		return -1;
	// find start of value
	if (fill_string_char(&rt, NULL, 0, '=') == -1)
		return -1;
	...
	if (fill_string_char(&rt, value, value_len, '&') == -1 && fill_string_char(&rt, value, value_len, '\0') == -1)
		return -1;
...
int korall_request_header_get(const HTTPRequest *req, const char *field, char *value, size_t value_len)
{
	...
	const char *base = h;
	bool is_case_insensitive = false;
	if (fill_string_str(&h, NULL, 0, field, is_case_insensitive) == -1)
		return -1;
	...
	if (fill_string_char(&h, NULL, 0, ':') == -1)
		return -1;
	h += 2; // skip : and space
...
int korall_response_start_set(HTTPResponse *res, const HTTPRequest *req, const int code)
{
	...
	{
		KORALL_LOG(LOG_ERR, "failed to set response, invalid code\n");
		return -1;
	}
...
int korall_response_header_set(HTTPResponse *res, const char *field, const char *value)
{
	...
	{
		KORALL_LOG(LOG_ERR, "failed to set header, field and value must not be NULL\n");
		return -1;
	}

	if (res->header_count >= res->header_capacity)
	{
		KORALL_LOG(LOG_ERR, "failed to set header, maximum of %zu headers reached\n", res->header_capacity);
		return -1;
	}
	...
	{
		KORALL_LOG(LOG_ERR, "failed to set header, must be under %zu characters total\n", res->header_size);
		return -1;
	}
	...
	{
		KORALL_LOG(LOG_ERR, "failed to set header, value is not valid for field %s\n", field);
		return -1;
	}
...
int korall_response_body_set(HTTPResponse *res, const char *body)
{
	...
	{
		KORALL_LOG(LOG_ERR, "failed to set body, body too long\n");
		return -1;
	}
	...
	if (korall_response_header_set(res, "Content-Length", cl) == -1)
		return -1;
