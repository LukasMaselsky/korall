...
int msg_queue_post(mqd_t mqdes, unsigned long thread_id, Message *msg)
{
	...
	Message *msg_copy = safe_calloc(1, msg_len);
	memcpy((void *)msg_copy, (const void *)msg, msg_len); // todo: free
	...
	if (ret == 0)
	{
		...
		case ERROR_INVALID_THREAD_ID:
			KORALL_LOG(LOG_ERR, "could not post message to queue, thread id invalid or thread does not have a message queue\n");
			break;
		case ERROR_NOT_ENOUGH_QUOTA:
			KORALL_LOG(LOG_ERR, "could not post message to queue, message limit hit\n");
			break;
		default:
			KORALL_LOG(LOG_ERR, "could not post message to queue\n");
			break;
		}
		return -1;
	}
#else
	if (mq_send(mqdes, (const char *)msg_copy, msg_len, 0) == -1)
	{
		KORALL_LOG(LOG_ERR, "could not post message to queue\n")
		return -1;
	}
...
int msg_queue_read(mqd_t mqdes, Message *out)
{
	...
	{
		memcpy((void *)out, (const void *)msg.lParam, out_len);
	}
	...
	ssize_t bytes_read = mq_receive(mqdes, (char *)out, out_len, NULL);
	if (bytes_read == -1)
	{
		KORALL_LOG(LOG_ERR, "mq_receive failed\n");
		return -1;
	}
